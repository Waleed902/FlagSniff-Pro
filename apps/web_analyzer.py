...
from scapy.all import rdpcap, PcapReader, TCP, IP, UDP, Raw, ICMP
import tempfile
import audioop
import struct
import io
...
from analyzers.protocols.icmp import analyze_icmp_packets, ICMPAnalyzer, ICMPTunnelDetector
...
class WebPcapAnalyzer:
    ...
    def analyze_file(self, file_path: str, search_options: Dict[str, bool], 
        ...
        try:
            ...
            # Analyze ICMP packets for tunneling/exfiltration
            if progress_callback:
                progress_callback("Analyzing ICMP packets...")
            icmp_results = analyze_icmp_packets(packets)
            if icmp_results.get('icmp_analysis', {}).get('total_packets', 0) > 0:
                self.results['icmp_analysis'] = icmp_results
                # Add ICMP findings
                for finding in icmp_results.get('summary', {}).get('key_findings', []):
                    f = Finding(
                        kind='icmp_' + finding['type'].lower().replace(' ', '_'),
            ...
            if progress_callback:
                progress_callback("Reconstructing TLS streams...")
            tls_results = reconstruct_tls_streams(packets)
            if tls_results.get('sessions'):
                self.results['tls_streams'] = tls_results
                # Add TLS findings
                for finding in tls_results.get('findings', []):
                    f = Finding(
                        kind='tls_' + str(finding.get('type','unknown')).lower().replace(' ', '_'),
                        data=finding.get('value', ''),
                        protocol='TLS',
                        confidence=0.7,
                        via='tls_reconstructor',
                        display_type='TLS',
                        icon='🔒'
                    )
                    self.results.setdefault('findings', []).append(asdict(f))
            ...
            # Run extended analyzer suites (IPv6/ICS/Crypto/RF/ML/DB/Malware/Temporal/Fingerprinting/DLP)
            if progress_callback:
                progress_callback("Running advanced analyzers...")
            try:
                ...
                try:
                    ...
                    ipv6_res = analyze_ipv6_traffic(packets)
                    tunnels_res = detect_ipv6_tunneling(packets)
                    icmpv6_res = analyze_icmpv6_packets(packets)
                    advanced['ipv6'] = ipv6_res
                    advanced['ipv6_tunnels'] = tunnels_res
                    advanced['icmpv6'] = icmpv6_res
                except Exception:
                    pass
                ...
                try:
                    ...
                    advanced['ml_anomalies'] = analyze_ml_anomalies(packets)
                except Exception:
                    pass
                ...
                try:
                    from analyzers.protocols.database import (
                        analyze_mysql_traffic,
                        analyze_postgres_traffic,
                        analyze_mongodb_traffic,
                        analyze_redis_traffic,
                        analyze_mssql_traffic,
                    )
                    advanced['databases'] = {
                ...
                try:
                    ...
                    advanced['malware'] = detect_malware_traffic(packets)
                except Exception:
                    pass
                ...
                try:
                    ...
                    advanced['temporal'] = analyze_time_patterns(packets)
                except Exception:
                    pass

                # Fingerprinting
                try:
                    from analyzers.fingerprinting import (
                        analyze_os_fingerprints,
                        analyze_service_fingerprints,
                    )
                ...
                try:
                    ...
                    advanced['dlp'] = analyze_dlp(packets)
                except Exception:
                    pass

                # Graph flows (count only to avoid heavy exports in web UI)
                try:
                    from exporters.neo4j_exporter import build_flows_from_packets
                    flows = build_flows_from_packets(packets)
                    advanced['flows'] = {'count': len(flows)}
                except Exception:
                    pass

                # Attach to results
                if advanced:
                    self.results['advanced'] = advanced

                # Surface a few high-signal findings into unified findings list
                try:
                    # Crypto mining
                    mining = ((advanced.get('blockchain') or {}).get('mining') or {})
                    sev = str((mining.get('severity') or '')).lower()
                    if sev in ('high', 'medium'):
                        desc = (mining.get('analysis') or {}).get('summary') or f"Crypto mining indicators detected (severity: {sev})"
                        self.results.setdefault('findings', []).append(asdict(Finding(
                            kind='crypto_mining', data=str(desc)[:300], protocol='STRATUM', confidence=0.8 if sev=='high' else 0.65,
                            display_type='MINING', icon='⛏️'
                        )))
                    # Malware suspicious patterns
                    mal = (advanced.get('malware') or {}).get('suspicious_patterns') or []
                    for m in mal[:5]:
                        self.results.setdefault('findings', []).append(asdict(Finding(
                            kind='malware_suspicion', data=str(m.get('description') or m)[:300], protocol='ANY', confidence=0.7,
                            display_type='MALWARE', icon='🦠'
                        )))
                    # DLP findings
                    dlp = (advanced.get('dlp') or {}).get('results') or []
                    for p in dlp[:5]:
                        ptype = str(p.get('type','PII')).upper()
                        self.results.setdefault('findings', []).append(asdict(Finding(
                            kind='pii', data=str(p.get('value',''))[:300], protocol='ANY', confidence=0.75,
                            display_type=f'DLP/{ptype}', icon='🛡️'
                        )))
                    # IPv6 suspicious
                    ipv6_susp = (advanced.get('ipv6') or {}).get('suspicious_patterns') or []
                    for s in ipv6_susp[:3]:
                        self.results.setdefault('findings', []).append(asdict(Finding(
                            kind='ipv6_alert', data=str(s.get('description') or s)[:300], protocol='IPv6', confidence=0.6,
                            display_type='IPv6', icon='🧭'
                        )))
                except Exception:
                    pass
                # Generate prioritized hints from advanced analysis
                try:
                    adv_hints, adv_summary = self._generate_advanced_hints(advanced)
                    if adv_hints:
                        self.results['advanced_ai_hints'] = adv_hints
                        # Also merge into ai_suggestions for unified AI tab display
                        existing = self.results.get('ai_suggestions') or []
                        # store as plain strings for ai_suggestions, but keep structured in advanced_ai_hints
                        merged = existing + [h.get('hint') or h.get('title') for h in adv_hints if h]
                        # dedupe while preserving order
                        seen_txt = set()
                        uniq_txt = []
                        for s in merged:
                            if s and s not in seen_txt:
                                seen_txt.add(s)
                                uniq_txt.append(s)
                        self.results['ai_suggestions'] = uniq_txt[:20]
                    if adv_summary:
                        self.results['advanced_summary'] = adv_summary
                except Exception:
                    pass
            except Exception:
                # Keep pipeline robust if any advanced analyzer fails
                pass
    ...
    def _generate_advanced_hints(self, advanced: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], str]:
        """Produce prioritized hints from advanced analyzer outputs.

        Returns (hints, summary_text). Hints are dicts with keys: title, hint, priority, confidence, category.
        """
        hints: List[Dict[str, Any]] = []
        pieces: List[str] = []
        try:
            ...
            # Crypto mining
            try:
                mining = ((adv.get('blockchain') or {}).get('mining') or {})
                sev = str((mining.get('severity') or '')).lower()
                if sev in ('high', 'medium'):
                    hints.append({
                        'title': 'Crypto mining indicators',
                        'hint': f"Potential crypto-mining traffic detected (severity: {sev}). Check Stratum connections and pool domains.",
                        'priority': 'high' if sev == 'high' else 'medium',
                        'confidence': 0.85 if sev == 'high' else 0.7,
                        'category': 'blockchain/mining'
                    })
                    pieces.append(f"mining={sev}")
            except Exception:
                pass
            ...
            try:
                ...
                if mal:
                    hints.append({
                        'title': 'Malware-like traffic patterns',
                        'hint': f"{len(mal)} suspicious malware patterns observed (e.g., DGA/beaconing/lateral movement). Review affected hosts.",
                        'priority': 'high',
                        'confidence': 0.8,
                        'category': 'malware'
                    })
                    pieces.append(f"malware={len(mal)}")
            except Exception:
                pass
            ...
            try:
                ...
                if dlp:
                    hints.append({
                        'title': 'PII exposure',
                        'hint': f"Detected {len(dlp)} PII matches (emails/SSN/credit cards). Investigate potential data leakage.",
                        'priority': 'high',
                        'confidence': 0.85,
                        'category': 'dlp'
                    })
                    pieces.append(f"pii={len(dlp)}")
            except Exception:
                pass

            # IPv6 tunnels / suspicious
            try:
                tunnels = len(((adv.get('ipv6_tunnels') or {}).get('detected_tunnels') or []) )
                if tunnels:
                    hints.append({
                        'title': 'IPv6 tunneling detected',
                        'hint': f"Found {tunnels} IPv6 tunnel indicators (e.g., 6in4/6to4/Teredo). Validate policy compliance.",
                        'priority': 'medium',
                        'confidence': 0.7,
                        'category': 'ipv6'
                    })
                    pieces.append(f"ipv6_tunnels={tunnels}")
                ipv6_susp = (adv.get('ipv6') or {}).get('suspicious_patterns') or []
                if ipv6_susp:
                    hints.append({
                        'title': 'IPv6 anomalies',
                        'hint': f"{len(ipv6_susp)} suspicious IPv6 patterns (e.g., fragments, deprecated headers).", 
                        'priority': 'medium',
                        'confidence': 0.65,
                        'category': 'ipv6'
                    })
                    pieces.append(f"ipv6_susp={len(ipv6_susp)}")
            except Exception:
                pass

            # ML anomalies (beaconing / outliers)
            try:
                ml = adv.get('ml_anomalies') or {}
                beacons = (ml.get('beacon_flows') or [])
                outliers = (ml.get('outlier_flows') or [])
                if beacons:
                    hints.append({
                        'title': 'Beacon-like traffic',
                        'hint': f"Detected {len(beacons)} flows with periodic beacon characteristics. Inspect timing and destinations.",
                        'priority': 'high',
                        'confidence': 0.75,
                        'category': 'ml'
                    })
                    pieces.append(f"beacons={len(beacons)}")
                if outliers:
                    hints.append({
                        'title': 'Anomalous flows',
                        'hint': f"Identified {len(outliers)} outlier flows via MAD statistics. Prioritize review.",
                        'priority': 'medium',
                        'confidence': 0.65,
                        'category': 'ml'
                    })
                    pieces.append(f"outliers={len(outliers)}")
            except Exception:
                pass

            # RF unusual behavior (deauth etc.)
            try:
                rf = adv.get('rf') or {}
                wifi = rf.get('wifi') or {}
                deauth = int(wifi.get('deauth_frames', 0)) if isinstance(wifi.get('deauth_frames'), (int, float)) else 0
                if deauth > 0:
                    hints.append({
                        'title': 'Wi‑Fi deauthentication activity',
                        'hint': f"Observed {deauth} deauth frames; possible wireless attack/testing. Correlate with timeframe.",
                        'priority': 'high',
                        'confidence': 0.8,
                        'category': 'rf/wifi'
                    })
                    pieces.append(f"deauth={deauth}")
            except Exception:
                pass

            # Database protocol presence
            try:
                db = adv.get('databases') or {}
                present = [k for k, v in db.items() if (v or {}).get('sessions') or (v or {}).get('connections') or (v or {}).get('total', 0) > 0]
                if present:
                    hints.append({
                        'title': 'Database protocol activity',
                        'hint': f"Detected DB protocols: {', '.join([p.upper() for p in present])}. Review auth and plaintext leaks.",
                        'priority': 'info',
                        'confidence': 0.55,
                        'category': 'database'
                    })
                    pieces.append(f"db={','.join(present)}")
            except Exception:
                pass
            ...
            try:
                ...
                os_guesses = fp.get('os') or []
                if os_guesses:
                    hints.append({
                        'title': 'Passive OS/service fingerprinting',
                        'hint': f"OS guesses available for {len(os_guesses)} hosts; may refine scoping and context.",
                        'priority': 'info',
                        'confidence': 0.6,
                        'category': 'fingerprint'
                    })
                    pieces.append(f"os={len(os_guesses)}")
            except Exception:
                pass

            # Prioritize
            pri_order = {'high': 3, 'medium': 2, 'info': 1}
            hints.sort(key=lambda h: (pri_order.get(h.get('priority','info'),1), float(h.get('confidence',0))), reverse=True)

            # Build compact summary line
            summary = ", ".join(pieces[:8]) if pieces else ""
            return hints[:12], summary
        except Exception:
            return [], ""
