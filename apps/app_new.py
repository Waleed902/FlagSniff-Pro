...
def render_analyzer_page():
    ...
    if uploaded_file:
        ...
        # Availability hint
        try:
            from tshark_ai import tshark_available as _tshark_available
...
def render_results_page():
    ...
    if st.session_state.analysis_results:
        ...
        with tabs[3]:
            ...
            # ICMP Analysis Section
            if results.get('icmp_analysis'):
                with st.expander("📡 ICMP Analysis", expanded=True):
                    icmp_data = results['icmp_analysis']
                    icmp_summary = icmp_data.get('icmp_analysis', {})
                    tunnel_data = icmp_data.get('tunnel_detection', {})
                    
                    col1, col2, col3 = st.columns(3)
                    col1.metric("Total ICMP Packets", icmp_summary.get('total_packets', 0))
                    col2.metric("Tunneling Detected", "Yes" if tunnel_data.get('detected') else "No")
                    col3.metric("Confidence", f"{tunnel_data.get('confidence', 0)}%")
                    
                    # Type distribution
                    if icmp_summary.get('type_distribution'):
                        st.markdown("**ICMP Type Distribution:**")
                        type_df = pd.DataFrame([
                            {'Type': k, 'Count': v} 
                            for k, v in icmp_summary['type_distribution'].items()
                        ])
                        st.dataframe(type_df, hide_index=True, use_container_width=True)
                    
                    # Suspicious patterns
                    if icmp_summary.get('suspicious_patterns'):
                        st.markdown("**⚠️ Suspicious Patterns:**")
                        for pattern in icmp_summary['suspicious_patterns']:
                            severity_icon = "🔴" if pattern['severity'] == 'high' else "🟡"
                            st.warning(f"{severity_icon} **{pattern['type']}**: {pattern['description']}")
                    
                    # Extracted data
                    if icmp_summary.get('extracted_data'):
                        st.markdown("**📦 Extracted Data:**")
                        for data in icmp_summary['extracted_data'][:10]:
                            with st.expander(f"{data['type']} ({data['size']} bytes)"):
                                if 'preview' in data:
                                    st.text(data['preview'])
                                if 'data' in data:
                                    st.code(data['data'][:500], language='text')
            
            # TLS Stream Analysis Section
            if results.get('tls_streams'):
                with st.expander("🔒 TLS Stream Analysis", expanded=True):
                    tls_data = results['tls_streams']
                    tls_summary = tls_data.get('summary', {})
                    
                    col1, col2, col3 = st.columns(3)
                    col1.metric("TLS Sessions", tls_summary.get('total_sessions', 0))
                    col2.metric("Completed Handshakes", tls_summary.get('completed_handshakes', 0))
                    col3.metric("Server Names Found", len(tls_summary.get('server_names', [])))
                    
                    # TLS versions
                    if tls_summary.get('tls_versions'):
                        st.markdown("**TLS Versions:**")
                        st.write(", ".join(tls_summary['tls_versions']))
                    
                    # Cipher suites
                    if tls_summary.get('cipher_suites'):
                        st.markdown("**Cipher Suites:**")
                        for suite in tls_summary['cipher_suites'][:10]:
                            st.write(f"- {suite}")
                    
                    # Server names (SNI)
                    if tls_summary.get('server_names'):
                        st.markdown("**🌐 Server Names (SNI):**")
                        for sni in set(tls_summary['server_names'])[:20]:
                            st.write(f"- `{sni}`")
                    
                    # Session details
                    if tls_data.get('sessions'):
                        st.markdown("**Session Details:**")
                        for stream_id, session in list(tls_data['sessions'].items())[:10]:
                            with st.expander(f"Stream: {stream_id}"):
                                st.write(f"**Packets:** {session.get('packets', 0)}")
                                st.write(f"**Handshake Complete:** {'Yes' if session.get('handshake_complete') else 'No'}")
                                
                                if 'version' in session:
                                    st.write(f"**Version:** {session['version']}")
                                if 'cipher_suite_name' in session:
                                    st.write(f"**Cipher Suite:** {session['cipher_suite_name']}")
                                if 'server_name' in session:
                                    st.write(f"**Server Name:** {session['server_name']}")
                                
                                if session.get('app_data_size', 0) > 0:
                                    st.write(f"**Application Data:** {session['app_data_size']} bytes")
                                
                                if session.get('alerts'):
                                    st.warning("**Alerts:**")
                                    for alert in session['alerts']:
                                        st.write(f"  - {alert.get('level')}: {alert.get('description')}")
...
def run_analysis(uploaded_file, search_options, custom_regex, ai_mode, ai_enabled, confidence_threshold, ctf_context=None, user_decrypt_key=None, tls_keylog_upload=None):
    ...
    try:
        ...
        if tls_keylog_path and os.path.exists(tls_keylog_path):
            os.unlink(tls_keylog_path)
