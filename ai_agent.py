    ...
    def __init__(self, api_key: str, model: str = "LongCat-Flash-Chat", base_url: str = None):
        ...
        self.specialist_modes = {
            ...
            "🔗 Encoding Chain Decoder": {
                ...
                "focus": ["base64", "XOR (bruteforcing key)", "rot13", "url_encoding", "multi_layer"],
                "confidence_boost": 20
            },
...
Respond with structured JSON containing your analysis, confidence scores, and detailed reasoning."""
    ...
    # --- Grounded, tool-using helpers to reduce hallucinations ---
    def grounded_explain(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """Summarize strictly from provided results dict; no external generation.

        Returns a compact structure with counts, notable examples, and suggested next actions
        that reference existing exports (reports, IOCs, streams ZIP).
        """
        try:
            res = results or {}
            ...
            flags = []
            for f in (res.get('ctf_analysis', {}).get('flag_candidates') or []):
            ...
            summary = {
                'counts': {
                    ...
                    'sessions': len(res.get('sessions') or {}),
                    'streams': len(res.get('reconstructed_streams') or {}),
                },
                'examples': {
                    'flags': [f.get('flag') for f in flags[:3]],
                    'ioc_preview': {k: (v[:5] if isinstance(v, list) else v) for k, v in (res.get('iocs') or {}).items()},
                    'decoded_preview': [ (d.get('decoded') or d.get('result',''))[:120] for d in decoded[:3] ],
                },
                'exports': {
                    'reports': (res.get('report_exports') or {}),
                    'iocs': (res.get('ioc_exports') or {}),
                    'graph': (res.get('graph_exports') or {}),
                    'streams_zip': (res.get('exports') or {}).get('streams_zip')
                },
                'suggested_next_actions': self._grounded_next_actions(res)
            }
            return {'advanced_explanation': summary, 'mode': 'grounded'}
        except Exception as e:
            return {'advanced_explanation': {'error': str(e)}, 'mode': 'grounded'}

    def _grounded_next_actions(self, results: Dict[str, Any]) -> List[Dict[str, Any]]:
        actions: List[Dict[str, Any]] = []
        try:
            if results.get('ioc_exports'):
                actions.append({'title': 'Review exported IOCs', 'ref': results['ioc_exports']})
            if results.get('report_exports'):
                actions.append({'title': 'Open HTML/PDF report', 'ref': results['report_exports']})
            if (results.get('exports') or {}).get('streams_zip'):
                actions.append({'title': 'Download streams ZIP', 'ref': results['exports']['streams_zip']})
            if results.get('replay_commands'):
                actions.append({'title': 'Run safe replay commands', 'count': len(results['replay_commands'])})
            if results.get('decoded_data'):
                actions.append({'title': 'Inspect top decoded outputs', 'count': len(results['decoded_data'])})
        except Exception:
            pass
        return actions

    def analyze_text_locally(self, text: str) -> Optional[Dict[str, Any]]:
        """Run local cryptanalysis engine on a text blob (no network)."""
        try:
            from features.cryptanalysis_suite import CryptanalysisEngine  # type: ignore
            eng = CryptanalysisEngine()
            return eng.analyze_text(text)
        except Exception:
            return None

    def analyze_binary_locally(self, data: bytes) -> Optional[Dict[str, Any]]:
        """Run local cryptanalysis engine on bytes (no network)."""
        try:
            from features.cryptanalysis_suite import CryptanalysisEngine  # type: ignore
            eng = CryptanalysisEngine()
            return eng.analyze_binary(data)
        except Exception:
            return None
...
Whitelisted actions: list_findings, show_decoded, show_ctf_flags, explain_stream, extract_stream_by_id, search_text, decode_text, list_sessions, search_decoded, show_jwts, verify_flags
